...
const Message = () => {
    ...
    //       fetchingallchat(); 
    //   },[])
    const { data, error, isLoading } = useSWR(`${URL}api/v1/chats`,
      async (url) => {
        const instance = axios.create({
          withCredentials: true,
          headers: { authorization: "Bearer" },
    ...
    <div className={styles.messageContainer}>
      <div className={styles.allchat}>
      {
        ...
        <>
          {
            allchat.map((data)=>{
                  return <div className={((data._id == currentchat?._id)?(styles.activeImage):(styles.notactiveImage))} key={data.id} onClick={() => colorchangeHandler(data)} >
                    <div className={styles.productdetailcontainer}>
                          <img src={data.productId.MainImage} alt={data.productId.id} className={styles.img} />
                          <div>
